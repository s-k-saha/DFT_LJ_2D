...
#define dz 0.025
#define dx 0.025
//#define mu_solvent -3.8214630890212753
//#define mu_solute -3.8214630890212753
...
double rho[3][Nx*Nz],rhonew[3][Nx*Nz],rhocopy[Nx*Nz],Vext[Nx*Nz],Ufilter[Nx*Nz];
...
void getomega3(),getomega2(),getomega1(),getomega0(),getomega1v(),getomega2v(),rhocpy(),filterrho(),write_rho(double,int),conv_FFT2D_2(double*,fftw_complex *,double*),getc1_LJ2(),getUFilterFFT(),getc1_LJ2_i(int,int,double),get_mu();
...
void iterate(){	
	...
	for(int i=0;i<Nx;i++)
		for(int j=0;j<Nz;j++)
			{
				rhonew[0][i*Nz+j]=exp(-Vext[i*Nz+j]+c1[0][i*Nz+j]+mu_solvent);
				rhonew[1][i*Nz+j]=exp(-Vext[i*Nz+j]+c1[1][i*Nz+j]+mu_solute);
				rhonew[2][i*Nz+j]=exp(-Vext[i*Nz+j]+c1[2][i*Nz+j]+mu_solute);
			}
...
{
	char fname[100];
	sprintf(fname,"../data/newrho2Deps%feps_i%few%fdx%fLx%f_QN.dat",eps,eps_i,ew,dx,Lx);
	FILE *F=fopen(fname,"w");
	for(int i=0;i<Nx;i++)
	{
		for(int j=0;j<Nz;j++)
			fprintf(F,"%d %d %f %f %f\n",i,j,rho[0][i*Nz+j],rho[1][i*Nz+j],rho[2][i*Nz+j]);
		fprintf(F,"\n");
	}
	fprintf(F,"------------------\n20 x %d cycles time: %f s\n",count_iter,elapsed);
	fclose(F);
	
	sprintf(fname,"../data/newrho1Deps%feps_i%few%fdx%fLx%f_QN.dat",eps,eps_i,ew,dx,Lx);
	F=fopen(fname,"w");
	
	for(int j=0;j<Nz;j++)
	fprintf(F,"%f %f %f %f\n",j*dz,rho[0][(Nx/2)*Nz+j],rho[1][(Nx/2)*Nz+j],rho[2][(Nx/2)*Nz+j]);
	fprintf(F,"------------------\n20 x %d cycles time: %f s\n",count_iter,elapsed);
	fclose(F);
}
...
void main(int argc,char *argv[])
{
	...
	/*
	fftw_complex *fft_f=fftw_alloc_complex((size_t)Nx * K);
	fftw_plan ff = fftw_plan_dft_r2c_2d(Nx,Nz, Ufilter, fft_f, FFTW_ESTIMATE);
  fftw_execute(ff);
  fftw_destroy_plan(ff);
  double dk=2.*PI/Lx;
  for(int i=0;i<K;i++)
  printf("%f %f %f\n",dk*i,fft_f[i][0]/N,fft_f[i*(K)]);
 	*/
  ...
  //write_rho(elapsed,0);
	...
	int i=(Nx/2);
	int j=(Nz/2);
	printf("%d %d %f(1.0) %f(0.5) %f(3.14) %f(0.5236) %f(0) %f(0) \n",i,j,n0[i*Nz+j]/rhob,n1[i*Nz+j]/rhob,n2[i*Nz+j]/rhob,n3[i*Nz+j]/rhob,n2vx[i*Nz+j]/rhob,n2vz[i*Nz+j]/rhob);*/
